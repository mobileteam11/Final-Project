...
#include "utility.h"
#include "gtsamUtility.h"
using namespace gtsam;

class OdometryAndMapping{
...
private:
    ...
    // Initialize odometry transformation
    nav_msgs::Odometry odomAftMapped;
    tf::StampedTransform aftMappedTrans;
    tf::TransformBroadcaster tfBroadcaster;
    // Initialize mapping point clouds
    // Key frame point clouds for loop closure
    ...
    int latestFrameID;
    // Near surrounding Pose and key frames
    ...
    // Store pose as point cloud for graphing
    pcl::PointCloud<PointType>::Ptr cloudKeyPoses3D;
    pcl::PointCloud<PointTypePose>::Ptr cloudKeyPoses6D;
    pcl::PointCloud<PointType>::Ptr surroundingKeyPoses;
    pcl::PointCloud<PointType>::Ptr surroundingKeyPosesDS;
    // Laser point for graphing
...
public:
        ...
        nh("~")
    {
        ...
        subLaserCloudCornerLast = nh.subscribe<sensor_msgs::PointCloud2>("/laser_cloud_corner_last", 2, &OdometryAndMapping::laserCloudCornerLastHandler, this);
        subLaserCloudSurfLast = nh.subscribe<sensor_msgs::PointCloud2>("/laser_cloud_surf_last", 2, &OdometryAndMapping::laserCloudSurfLastHandler, this);
        subOutlierCloudLast = nh.subscribe<sensor_msgs::PointCloud2>("/outlier_cloud_last", 2, &OdometryAndMapping::laserCloudOutlierLastHandler, this);
        subLaserOdometry = nh.subscribe<nav_msgs::Odometry>("/laser_odom_to_init", 5, &OdometryAndMapping::laserOdometryHandler, this);
        subImu = nh.subscribe<sensor_msgs::Imu> (imuTopic, 50, &OdometryAndMapping::imuHandler, this);
    ...
    void run(){
            ...
            newLaserOdometry)
        {
            ...
            // mapping process interval 0.3
            // lower freqeucny for mapping
            if (timeLaserOdometry - timeLastProcessing >= mappingProcessInterval) {
                timeLastProcessing = timeLaserOdometry;
                // transfor point clouds to world coordinates
                transformLidarToMap();
                // feature matching for only surrounding key frames stored
                featureExtractionAndMatching();
                // add point clouds to map
                // estimate pose transform of the step
                twoStepLMOptimization();
                // store key frames for loop closure dectection
                saveKeyFramesAndFactor();
                // correct sensor pose and publish
                publishPosesAndFramesData();
                clearCloud();
            }
        }
    }
    ...
    void transformLidarToMap() // Transform Sensor Coordinates to World Map
    {
    ...
    void featureExtractionAndMatching(){
        extractSurroundingKeyFrames();
        downsampleCurrentScan();
    }

    void twoStepLMOptimization(){
        // 2 step LM Optimization
    ...
    void saveKeyFramesAndFactor(){
        ...
        // pose separation cannot be too close
        ...
        // otherwise, update pose based on the previous one
        else{
    ...
    void publishPosesAndFramesData(){
        correctPoses();
        publishTF();
    ...
    void extractSurroundingKeyFrames(){
        ...
        if (cloudKeyPoses3D->points.empty() == true)
            return; 
        // detect loop closure
        if (loopClosureEnableFlag == true){// if loop closure detected
            // surroundingKeyframeSearchNum =50
            // fill edge and planar features to enough
            ...
            }else{ // accumulate new point clouds
        ...
        }else{// if no loop closure
            surroundingKeyPoses->clear();
            surroundingKeyPosesDS->clear();
            // search near points within 50m stored in KD tree
    ...
    void updatePointLidarToMapSinCos(){
    ...
    void pointLidarToMap(PointType const * const pi, PointType * const po)
    {
    ...
    void cornerOptimization(int iterCount){
        updatePointLidarToMapSinCos();
        for (int i = 0; i < laserCloudCornerLastDSNum; i++) {
            pointOri = laserCloudCornerLastDS->points[i];
            pointLidarToMap(&pointOri, &pointSel);
    ...
    // Loop Closure and Global Map Visualization
    void loopClosure(){
